...
import h5py
import os
from Agents import DistributionalDuelingDQN
...
policy_net = DistributionalDuelingDQN(n_observations, n_actions).to(device)
target_net = DistributionalDuelingDQN(n_observations, n_actions).to(device)
...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            # Get the distribution from the policy network
            distribution = policy_net(state)

            # Compute the expected Q-values from the distribution
            z = torch.linspace(policy_net.v_min, policy_net.v_max, policy_net.n_atoms).to(state.device)
            q_values = torch.sum(distribution * z, dim=2)  # Shape: (batch size, n_actions)

            # Select the action with the maximum Q-value
            return q_values.max(1).indices.view(1, 1)  # Return the action index
    else:
        return torch.tensor([[env.action_space.sample()]], device=device, dtype=torch.long)

episode_rewards = []

def plot_rewards(show_result=False, save_dir="images/", save_path="training_rewards_C51DDQN_cartpole_curriculum_3.png"):
    plt.figure(1)
...
def optimize_model():
    ...
    # Atom values (z)
    z = torch.linspace(policy_net.v_min, policy_net.v_max, policy_net.n_atoms).to(device)

    # Get the distribution and compute expected Q-values for state-action pairs
    distribution = policy_net(state_batch)  # Shape: (batch_size, n_actions, n_atoms)
    q_values = torch.sum(distribution * z, dim=2)  # Shape: (batch_size, n_actions)

    # Select Q-values for the actions in the batch
    state_action_values = q_values.gather(1, action_batch)

    # Compute next state values
    next_state_values = torch.zeros(BATCH_SIZE, device=device)
    with torch.no_grad():
        # Compute the distribution for non-final states using target_net
        next_distribution = target_net(non_final_next_states)  # Shape: (batch_size, n_actions, n_atoms)
        next_q_values = torch.sum(next_distribution * z, dim=2)  # Shape: (batch_size, n_actions)

        # Use policy_net to select the best action for the next states
        next_action_indices = policy_net(non_final_next_states).sum(2).argmax(1)  # Argmax over Q-values

        # Use the target network to evaluate the value of the best actions
        next_state_values[non_final_mask] = next_q_values.gather(1, next_action_indices.unsqueeze(1)).squeeze(1)
...
if __name__ == "__main__":       
    ...
    # Save the trained policy_net model
        def save_model(model, save_dir, file_name="C51DDQN_cartpole_curriculum_3.h5"):
            # Ensure the directory exists
            os.makedirs(save_dir, exist_ok=True)
    ...
    plot_rewards(show_result=True, save_path="training_rewards_C51DDQN_cartpole_curriculum_3.png")
    save_model(policy_net, save_dir="models/", file_name="C51DDQN_cartpole_curriculum_3.h5")
    print("Model saved as C51DDQN_cartpole_curriculum_3.h5")
    plt.ioff()
    plt.show()
