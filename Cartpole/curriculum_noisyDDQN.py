...
import h5py
import os
from Agents import NoisyDuelingDQN
...
policy_net = NoisyDuelingDQN(n_observations, n_actions).to(device)
target_net = NoisyDuelingDQN(n_observations, n_actions).to(device)
...
def plot_rewards(show_result=False, save_dir="images/", save_path="training_rewards_noisyDDQN_curriculum.png"):
    plt.figure(1)
...
if __name__ == "__main__":        
    ...
    # Save the trained policy_net model
        def save_model(model, save_dir, file_name="noisyDDQN_cartpole_curriculum.h5"):
            # Ensure the directory exists
            os.makedirs(save_dir, exist_ok=True)
    ...
    plot_rewards(show_result=True, save_path="training_rewards_noisyDDQN_curriculum_3.png")
    save_model(policy_net, save_dir="models/", file_name="noisyDDQN_cartpole_curriculum_3.h5")
    print("Model saved as noisyDDQN_cartpole_curriculum_3.h5")
    plt.ioff()
    plt.show()
