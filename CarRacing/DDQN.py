...
# Testing params
PRETRAINED_PATH         = "model/Eajun/DDQN2/NONOISE/20241208-130704/episode_400.weights.h5"
...
class DDQN_Agent:
    ...
    def save( self, name, data ):
        ...
        # plotting results
        if PLOT_RESULTS: plot_rewards( f"{REWARD_DIR}" + name + ".csv", noise_type="Base" )
...
def train_agent(agent: DDQN_Agent, env: gym.make, episodes: int):
    ...
    for episode in tqdm(range(episodes)):
        ...
        if episode % SAVE_TRAINING_FREQUENCY == 0:
            agent.save(f"episodebla_{episode}.weights", data=episode_rewards)
...
if __name__ == "__main__":
    ...
    if not TEST:
        # Train Agent
        agent = DDQN_Agent()
        train_agent( agent, base_env, episodes = EPISODES )
