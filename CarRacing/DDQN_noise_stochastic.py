...
# Testing params
PRETRAINED_PATH         = "model/Eajun/DDQN2/STOCHASTIC/20241206-014040/episode_400.weights.h5"
...
class RandomizedCarRacingStochastic(gym.Wrapper):
    def __init__(self, env):
        super().__init__(env)
        self.env = env
        self.episode_count = 0  # Track the current episode

        # Initialize noise probabilities and ranges
        self.noise_probabilities = [0.3, 0.3, 0.3]  # Probabilities for each noise type (wind, friction, Gaussian)
        self.friction_range = (0.05, 0.2)
        self.wind_range = (-0.4, 0.4)
        self.gaussian_range = (0.0, 0.5)

        # Randomize environment-level parameters
    ...
    def apply_noise_stochastically(self, steering, brake, observation):
        """Apply stochastic noise to actions and observations."""
        # Wind force noise
        ...
        if np.random.random() < self.noise_probabilities[2]:
            ...
            # Cast observation to float, add noise, clip, and cast back to uint8
            observation = observation.astype(np.float32) + gaussian_noise
            observation = np.clip(observation, 0, 255).astype(np.uint8)
            print(f"Stochastic Gaussian Noise Applied: {gaussian_noise}")

        return steering, brake, observation
    ...
    def step(self, action):
        ...
        # Apply stochastic noise to actions and observation
        steering, brake, observation = self.apply_noise_stochastically(steering, brake, observation)

        # Clip and ensure the observation is within valid bounds
        observation = np.clip(observation, 0, 255).astype(np.uint8)
...
class DDQN_Agent:
    ...
    def save( self, name, data ):
        ...
        # plotting results
        if PLOT_RESULTS: plot_rewards( f"{REWARD_DIR}" + name + ".csv", noise_type="Stochastic" )
...
if __name__ == "__main__":
    ...
    env = RandomizedCarRacingStochastic(base_env)
