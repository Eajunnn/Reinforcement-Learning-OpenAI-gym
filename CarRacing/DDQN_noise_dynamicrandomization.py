...
# Testing params
PRETRAINED_PATH         = "model/Eajun/DDQN2/RANDOM/20241206-013930/episode_400.weights.h5"
TEST                    = True    # true = testing, false = training


############################## MAIN CODE BODY ##################################
class RandomizedCarRacing(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        self.randomize_environment()
        self.friction = np.random.uniform(0.05, 2)  # Reset friction in range [0.1, 0.5]
        self.wind_force = np.random.uniform(-0.4, 0.4)   # Reset wind force in range [-1, 1]
...
class DDQN_Agent:
    ...
    def save( self, name, data ):
        ...
        # plotting results
        if PLOT_RESULTS: plot_rewards( f"{REWARD_DIR}" + name + ".csv", noise_type="Random")
...
if __name__ == "__main__":
    ...
    env = RandomizedCarRacingTesting(base_env)
    ...
    if not TEST:
        ...
        # Test Agent
        agent = DDQN_Agent()
        test_agent( agent, env, model = PRETRAINED_PATH, testnum=10 )
